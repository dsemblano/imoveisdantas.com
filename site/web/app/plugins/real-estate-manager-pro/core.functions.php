...
function rem_render_field($field){
	...
	switch ($field['type']) {
		...
		case 'date':
			...
			echo '<input '.$required.' '.$max.' '.$min.' id="'.$field['key'].'" class="form-control input-sm" type="'.$field['type'].'" name="rem_property_data['.$field['key'].']" value="'.stripcslashes($value).'">';
			break;
			...
			<select <?php echo $required; ?> id="<?php echo $field['key'] ?>" name="rem_property_data[<?php echo $field['key']; ?>]" class="form-control input-sm">
				...
				<?php
					...
					foreach ($options as $name) {
						...
						echo '<option value="'.$name.'" '.selected( $value, $name, false ).'>'.$translated_label.'</option>';
					}
				?>
			</select>
			<?php 
		...
		case 'select2':
			?>
			<select <?php echo $required; ?> multiple id="<?php echo $field['key'] ?>" name="rem_property_data[<?php echo $field['key']; ?>][]" class="form-control input-sm rem-select2-field">
				<?php
					...
					foreach ($options as $name) {
						...
						echo '<option  '.$selected.' value="'.$name.'" >'.$translated_label.'</option>';
					}
				?>
			</select>
			<?php 
								...
								foreach ($value as $id) {
									...
									<div class="col-sm-3">
										<div class="rem-preview-image">
											<input type="hidden" name="rem_property_data[<?php echo $field['key']; ?>][<?php echo trim($id); ?>]" value="<?php echo $id; ?>">
		...
		case 'checkbox':
			...
			<div class="onoffswitch">
			    <input type="checkbox" <?php checked( $value, 'on', true); ?> value="on" name="rem_property_data[property_detail_cbs][<?php echo $feature_key; ?>]" class="onoffswitch-checkbox" id="<?php echo $field['key']; ?>">
	...
	switch ($key) {
		case 'order': ?>
			<select class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="order" id="order">
				<option value=""><?php echo $dashes; ?> <?php _e( 'Order', 'real-estate-manager' ); ?> <?php echo $dashes; ?></option>
		...
		case 'tags': ?>
			<select class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="tag" id="tag">
				<option value=""><?php echo $dashes; ?> <?php _e( 'Tag', 'real-estate-manager' ); ?> <?php echo $dashes; ?></option>
				<?php 
					$tags = get_terms( 'rem_property_tag' );
					foreach ($tags as $tag_ob) { ?>
						<option value="<?php echo $tag_ob->term_id; ?>" <?php echo (isset($_GET['tag']) && $_GET['tag'] == $tag_ob->term_id) ? 'selected' : '' ; ?>><?php echo $tag_ob->name; ?></option>
					<?php }
				?>
			</select>
			<?php break;
		...
		case 'orderby': ?>
			<select class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="orderby" id="orderby">
				<option value=""><?php echo $dashes; ?> <?php _e( 'Order By', 'real-estate-manager' ); ?> <?php echo $dashes; ?></option>
		...
		case 'agent': ?>
			<select class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="agent_id" id="agent_id">
				<option value=""><?php echo $dashes; ?> <?php _e( 'Agent', 'real-estate-manager' ); ?> <?php echo $dashes; ?></option>
				<?php 
					$all_agents = get_users( 'role=rem_property_agent' );
					foreach ($all_agents as $agent) { ?>
						<option value="<?php echo $agent->ID; ?>" <?php echo (isset($_GET['agent_id']) && $_GET['agent_id'] == $agent->ID) ? 'selected' : '' ; ?>><?php echo rem_wpml_translate($agent->display_name, 'Agent', 'display_name_'.$agent->ID); ?></option>
					<?php }
				?>
			</select>
			<?php break;
...
function rem_render_property_search_fields($field, $display = 'widget'){
	...
	if (has_action( "rem_render_property_search_field_{$field['key']}" )) {
		...
		<select class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="<?php echo $field['key']; ?>" id="<?php echo $field['key']; ?>">
			<option value=""><?php echo $dashes; ?> <?php echo ($display == 'widget') ? __( 'Any', 'real-estate-manager' ) : rem_wpml_translate($field['title'], 'real-estate-manager-fields'); ?> <?php echo $dashes; ?></option>
				<?php
	...
	<?php } elseif ($field['type'] == 'select') { ?>
		<select class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="<?php echo $field['key']; ?>" id="<?php echo $field['key'].$display; ?>">
			<option value=""><?php echo $dashes; ?> <?php echo ($display == 'widget') ? __( 'Any', 'real-estate-manager' ) : rem_wpml_translate($field['title'], 'real-estate-manager-fields'); ?> <?php echo $dashes; ?></option>
				<?php
					foreach ($field['options'] as $title) {
						...
						echo '<option value="'.trim($title).'" '.$selected.'>'.rem_wpml_translate($title, 'real-estate-manager-fields').'</option>';
					}
				?>
		</select>
		<?php } elseif ($field['type'] == 'select2') { ?>
		<select multiple class="rem-select2-field" name="<?php echo $field['key']; ?>[]" id="<?php echo $field['key'].$display; ?>[]" data-placeholder="<?php echo $dashes; ?> <?php echo ($display == 'widget') ? __( 'Any', 'real-estate-manager' ) : $field['title']; ?> <?php echo $dashes; ?>">
			<option value=""><?php echo $dashes; ?> <?php echo ($display == 'widget') ? __( 'Any', 'real-estate-manager' ) : $field['title']; ?> <?php echo $dashes; ?></option>
				<?php
					foreach ($field['options'] as $title) {
						$title = stripcslashes($title);
						$selected = '';
    	...
    	if(rem_get_option('prefill_search_fields', '') == 'enable'){
		    ...
		    if ( $the_query->have_posts() ) :
				...
				<input class="form-control" type="text" placeholder="<?php echo rem_wpml_translate($field['title'], 'real-estate-manager-fields'); ?>" name="<?php echo $field['key']; ?>" id="<?php echo $field['key']; ?>" value="<?php echo (isset($_GET[$field['key']])) ? esc_attr( $_GET[$field['key']] ) : '' ; ?>"/>
		    <?php endif;		    
    	} else { ?>
			<input class="form-control" type="text" placeholder="<?php echo rem_wpml_translate($field['title'], 'real-estate-manager-fields'); ?>" name="<?php echo $field['key']; ?>" id="<?php echo $field['key']; ?>" value="<?php echo (isset($_GET[$field['key']])) ? esc_attr( $_GET[$field['key']] ) : '' ; ?>"/>
    	<?php }
...
function rem_render_property_dropdown_fields($field, $fixed_fields){
	...
	if ($field['type'] == 'select' || $field['type'] == 'select2' ) { ?>
		<div class="rem-ajax-search-widget" data-settings='{"cutOff": 5}' name="<?php echo $field['key']; ?>" id="<?php echo $field['key'].$display; ?>">
			<?php
			foreach ($field['options'] as $key => $title) {
				...
				echo '<div class="checkbox"><label><input type="checkbox" name="'.$field['key'].'[]" value="'.$title.'"> '.rem_wpml_translate($title, 'real-estate-manager-fields').' ('.$query->found_posts.')</label></div>';
			}
			?>
		</div>
		<?php }
...
function rem_render_price_range_field($display = 'widget'){
	...
	<?php } else if (rem_get_option('search_price_range', 'slider') == 'min_max_dropdown') { ?>
		<select class="<?php echo $dropdown_class; ?> rem_price_dropdown" data-settings='{"cutOff": 5}' >
			<option value=""><?php echo $dashes; ?> <?php echo __( 'Price', 'real-estate-manager' ); ?> <?php echo $dashes; ?></option>
				<?php
				if (!empty(rem_get_option('price_dropdown_options'))) {
					...
					foreach ($options as $value) {
						...
						$max = !empty($price[1]) ? $price[1]: 99999;
...
function rem_render_section_title($tabData){
	$title = __( $tabData['title'], 'real-estate-manager' );
	$tab_key = $tabData['key'];
	$icon = '';
	...
	$wrap = apply_filters( 'rem_property_section_title_wrap', 'h3', $tab_key );
...
function rem_render_range_field($field, $min, $max){
    ...
    <div class="p-slide-wrap">
        <div class="slider slider-range-input" data-default_min="<?php echo $min; ?>" data-default_max="<?php echo $max; ?>" data-max="<?php echo $max; ?>" data-min="<?php echo $min; ?>"></div>
        <div class="price-slider price <?php echo ($cb_label != '') ? 'text-right' : 'text-center' ; ?>">
    		<?php
    			...
    			if ($cb_label != '') {
        			...
        			echo '<input type="checkbox" '.$checkbox_check.'>';
        			echo apply_filters( 'rem_range_cb_label', $cb_label, $field );
        			echo '</label>';
    			}
    		?>
            <span id="price-value-min" class="price-value-min"></span>
            <span class="separator"><?php echo $field['title']; ?></span>
            <span id="price-value-max" class="price-value-max"></span>
        </div>
        <input type="hidden" name="range[<?php echo $field['key']; ?>][min]" class="min-value">
        <input type="hidden" name="range[<?php echo $field['key']; ?>][max]" class="max-value">
    </div>
<?php }
