...
<?php if (rem_get_option('energy_eff') == 'enable') {
	...
	foreach ($ef_classes as $key => $ef_c) {
		if ($key < 5) { ?>
			.rem-marker-energy[data-ef='<?php echo $ef_c; ?>'] {
			  left: 0
			}
			.rem-marker-energy[data-ef='<?php echo $ef_c; ?>']:before {
			  right: auto;
			  left: 0;
...
?>
	.logged-in.single-rem_property .ich-settings-main-wrap  .rem_sticky_sidebar {
	    ...
	    position: sticky;
	    top: calc(<?php echo $sticky_offset; ?>px + 32px);
	}
	.single-rem_property .ich-settings-main-wrap  .rem_sticky_sidebar {
	    top: <?php echo $sticky_offset; ?>px;
	    position: -webkit-sticky;
	    position: sticky;
	}
