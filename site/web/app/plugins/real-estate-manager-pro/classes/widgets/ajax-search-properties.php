...
class REM_AJAX_Search_Property_Widget extends WP_Widget {
		...
		extract($instance); ?>
			...
			<div class="search-box-page">
				...
				<div class="row rem-widget-search">
					<form class="rem_ajax_search_property_form" data-result_area_id="<?php echo $instance['result_area_id'] ?>">
						...
						<?php foreach ($all_fields as $field) { if ( isset( $instance[$field['key']] ) ): ?>
							<div class="col-md-12 space-div">
								<label for="<?php echo $field['key']; ?>"><?php echo rem_wpml_translate($field['title'], 'real-estate-manager-fields'); ?></label>
	...
	public function form( $instance ) {
		...
		<?php }} ?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'fixed_fields' ) ); ?>"><?php _e( 'Fixed Fields','real-estate-manager' ); ?></label> 
			<textarea
				class="widefat"
			...
			<br>
			<i><?php _e( 'Provide data for fixed fields on each line. Eg: property_status|normal', 'real-estate-manager' ); ?></i>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'result_area_id' ) ); ?>"><?php _e( 'Results Selector','real-estate-manager' ); ?></label> 
			<input
				class="widefat"
			...
			>
			<i><?php _e( 'Provide HTML selector where you want to display search results on current page', 'real-estate-manager' ); ?></i>
		</p>
		<?php 
	}
