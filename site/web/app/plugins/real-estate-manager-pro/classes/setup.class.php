...
class WCP_Real_Estate_Management
{
    ...
    function __construct(){
        ...
        add_action( 'edited_rem_property_cat', array( $this, 'updated_category_image' ), 10, 2 );
    ...
    */
    ...
    function admin_scripts($check){
        ...
        if ( $check == 'post-new.php' || $check == 'post.php' ) {
            ...
            if (isset($post->post_type) && 'attachment' === $post->post_type && isset($_GET['rem_image_editor'])) {
        ...
        if ($check == 'rem_property_page_rem_custom_fields') {
            ...
            if ( !defined("REM_FIELDS_PATH")  ) {
                wp_enqueue_script( 'rem-save-settings-page', REM_URL . '/assets/admin/js/property-fields.js'  , array( 'jquery', 'jquery-ui-accordion', 'jquery-ui-sortable', 'jquery-ui-draggable' ));
            }
        }
        ...
        if ($check == 'rem_property_page_rem_custom_field_sections') {
            ...
            wp_enqueue_script( 'rem-field-sections-builder', REM_URL . '/assets/admin/js/field-sections.js', array('jquery','jquery-ui-sortable', 'jquery-effects-highlight') );
        }
    ...
    function rem_role_cap(){
        ...
        if(get_option('rem_version_change') != 'done' && get_option( 'rem_property_fields' ) != ''){
    ...
    function rem_agent_extra_fields($user){
        ...
        if ($user->ID == $current_user->ID) { ?>
            <div style="text-align:center;">
                <h2><?php _e( 'Scan this QR code to login from the mobile application.', 'real-estate-manager' ); ?></h2>
                <p>
                    <a class="button" target="_blank" href="https://play.google.com/store/apps/details?id=com.webcodingplace.rem">Download Android App</a>
                    <a class="button" target="_blank" href="https://apps.apple.com/us/app/real-estate-manager-pro/id1530321208">Download iOS App</a>
                </p>
                <?php
    ...
    function menu_pages(){
        ...
        add_submenu_page( 'edit.php?post_type=rem_property', 'Real Estate Manager - Custo5m Fields Section', __( 'Field Sections', 'real-estate-manager' ), 'manage_options', 'rem_custom_field_sections', array($this, 'render_custom_field_sections_page') );
        ...
        add_submenu_page( 'edit.php?post_type=rem_property', 'Real Estate Manager - Documentation', __( 'Tools / Help', 'real-estate-manager' ), 'manage_options', 'rem_documentation', array($this, 'render_docs_page') );
    }
    ...
    function approve_agent(){
        if (isset($_REQUEST) && current_user_can( 'manage_options' )) {
            ...
            if (!empty($new_agent['agent_longitude'])) {
                update_user_meta( $agent_id, 'agent_longitude', $new_agent['agent_longitude']);
            }
            if (!empty($new_agent['agent_latitude'])) {
                update_user_meta( $agent_id, 'agent_latitude', $new_agent['agent_latitude']);
            }
    ...
    function save_field_sections(){
        ...
        if (isset($_REQUEST['sections'])) {
            ...
            if ($updated) {
                ...
                $resp = array(
                    'status' => 'success',
                    'title' => __( 'Sections Saved!', 'real-estate-manager' ),
                    'message' => __( 'Sections are saved in the database successfully.', 'real-estate-manager' )
                );
    ...
    function render_property_permalink_field(){
        ...
        <table class="form-table">
            <tr>
                <th><label for="rem_property_permalink"><?php _e( 'Property Page Base' , 'real-estate-manager' ); ?></label></th>
            ...
            <tr>
                <th><label for="rem_category_permalink"><?php _e( 'Property Category Base' , 'real-estate-manager' ); ?></label></th>
            ...
            <tr>
                <th><label for="rem_tag_permalink"><?php _e( 'Property Tag Base' , 'real-estate-manager' ); ?></label></th>
            ...
            foreach ($field_keys as $field_key) {
                if ($field_key != '') {
                    ...
                    if (isset($field_data['options']) && $field_data['options'] != '') {
                        ...
                        $selected = (isset($_REQUEST['rem_filter'][$field_key])) ? $_REQUEST['rem_filter'][$field_key] : '' ;
