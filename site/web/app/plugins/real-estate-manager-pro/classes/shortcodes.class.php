...
class REM_Shortcodes
{
	...
	function __construct(){
        ...
        add_action('elementor/widgets/widgets_registered', array($this, 'rem_elementor_widgets' ));

        /***********************************************************************************************/
        /* Shortcodes button in the tiny mce editor */
        /***********************************************************************************************/
	...
	function edit_property($attrs, $content = ''){
		...
		if (!is_admin() && is_user_logged_in() && isset($_GET['property_id']) && get_post_field( 'post_author', $_REQUEST['property_id'] ) == $current_user_data->ID) {
			extract( shortcode_atts( array(
				'style' => '',
    ...
    function change_password(){
    	if (isset($_REQUEST['old_pass']) && isset($_REQUEST['new_pass'])) {
    		...
    		if(is_wp_error($auth)) {
    			$resp = array('status' => 'error', 'message' => strip_tags($auth->get_error_message()), 'title' => __( 'Failed', 'real-estate-manager' ));
    		} else {
    ...
    function render_registration_field($field){
		...
		<li>
			...
			<?php switch ($field['type']) {
				...
				case 'select': ?>
					<select class="form-control" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?>>
				...
				case 'image': ?>
					<?php if (is_user_logged_in()) {  ?>
						<p class="text-center">
							<button type="button" class="btn btn-default upload_image_button" data-title="<?php _e( 'Select image', 'real-estate-manager' ); ?>" data-btntext="<?php _e( 'Insert', 'real-estate-manager' ); ?>">
					...
					?>
						<p class="text-center">
							<input data-size="<?php echo esc_attr( $max_size ); ?>" data-types='<?php echo json_encode($file_types); ?>' id="<?php echo $field['key']; ?>" type="file" accept="image/*" class="form-control" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
						</p>
			...
			<?php switch ($field['type']) {
				case 'text': ?>
					<?php if ($field['key'] == 'display_name') { $user_info = get_userdata($agent_id); ?>
						<input class="form-control" <?php echo $field_attrs; ?> type="text" value="<?php echo $user_info->display_name; ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
						<i class="icon fas fa-pencil-alt"></i>
					<?php } else { ?>
						<input class="form-control" <?php echo $field_attrs; ?> type="text" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
						<i class="icon fas fa-pencil-alt"></i>
					<?php } ?>
					<?php break;
				case 'number': ?>
					<input class="form-control" <?php echo $field_attrs; ?> type="number" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					<i class="icon fas fa-pencil-alt"></i>
					<?php break;
				case 'email':
					$user_info = get_userdata( $agent_id ); ?>
					<input class="form-control" <?php echo $field_attrs; ?> type="email" value="<?php echo $user_info->user_email; ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					<i class="icon fas fa-pencil-alt"></i>
					<?php break;
				case 'select': ?>
					<select class="form-control" <?php echo $field_attrs; ?> name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?>>
						<?php
						$options = explode("\n", $field['options']);
						...
						foreach ($options as $op) {
                            $op = trim($op);
							echo '<option '.selected( $val, $op, false ).' value="'.$op.'">'.$op.'</option>';
						} ?>
					</select>
					<?php break;
				case 'image': ?>
					<input class="form-control img-url" <?php echo $field_attrs; ?> type="text" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
					...
					<?php break;
				case 'textarea': ?>
					<textarea class="form-control" <?php echo $field_attrs; ?> rows="4" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?>><?php echo get_user_meta( $agent_id, $field['key'] , true ); ?></textarea>
					<p><?php echo rem_wpml_translate($field['help'], 'real-estate-manager-fields'); ?></p>
					<?php break;
					...
					<select <?php echo $required; ?> class="<?php echo esc_attr($dropdown_class); ?>" data-settings='{"cutOff": 5}' name="<?php echo $field['key']; ?>">
						...
						<?php
							...
							foreach ($options as $title) {
								...
								echo '<option value="'.$title.'" '.$selected.'>'.rem_wpml_translate($title, 'real-estate-manager-fields').'</option>';
							}
						?>
					</select>
					<?php break;
    ...
    function rem_elementor_widgets(){
		...
		if ( rem_get_option('elementor_theme_builder', 'enable') == 'enable' ) {
            rem_load_bs_and_fa();
            rem_load_basic_styles();
            $gallery_type = rem_get_option('gallery_type', 'fotorama');
