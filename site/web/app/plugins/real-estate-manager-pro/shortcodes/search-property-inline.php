					...
					<div class="row margin-div <?php echo ($disable_eq_height != 'yes') ? 'wcp-eq-height' : '' ; ?>">
						...
						<?php foreach ($default_fields as $field) {
							...
							if (in_array($field['key'], $fields_arr_inline) && 'property_price' != $field['key']){ ?>
								<div class="col-sm-hidden col-md-2 margin-bottom search-field" data-condition_status="<?php echo $show_condition; ?>" data-condition_bound="<?php echo isset($field['condition_bound']) ? $field['condition_bound'] : 'all' ?>" data-condition='<?php echo json_encode($conditions); ?>'>
