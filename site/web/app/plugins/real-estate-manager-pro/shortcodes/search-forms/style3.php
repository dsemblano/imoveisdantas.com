	...
	<?php if (in_array($tabs_field, $fields_arr)) {
	 ...
	 ?>
		<ul id="rem-search-tab" class="rem-search-tab justify-content-center">
	        ...
	        <?php
	        	if (is_array($options)) {
	        		foreach ($options as $option) { ?>
					    <li class="rem-search-tab-item">
					        <a class="rem-search-tab-link" data-val="<?php echo $option; ?>" href="#">
	        ...
	        ?>
		    <input type="hidden" name="<?php echo $tabs_field; ?>" value="">
		</ul>
	<?php } ?>
		...
		<div class="searcher">
			<div class="row <?php echo ($disable_eq_height == '' || $disable_eq_height == 'enable') ? 'wcp-eq-height' : '' ; ?>">
				...
				<?php if (in_array('search', $fields_arr)) { ?>
					<div class="col-sm-6 col-md-<?php echo $columns; ?> padding-none margin-bottom">
				...
				foreach ($default_fields as $field) {
					...
					if (in_array($field['key'], $fields_arr) && 'property_price' != $field['key']){ ?>
						<div class="col-sm-6 col-md-<?php echo $columns; ?> padding-none margin-bottom search-field" data-condition_status="<?php echo $show_condition; ?>" data-condition_bound="<?php echo isset($field['condition_bound']) ? $field['condition_bound'] : 'all' ?>" data-condition='<?php echo json_encode($conditions); ?>'>
				...
				<?php if (in_array('property_price', $fields_arr)) { ?>
					<div class="p-slide-wrap col-sm-6 col-md-<?php echo $columns ?> padding-none margin-bottom">
...
<script type="text/javascript">
	jQuery(function($) {
        ...
        $('#rem-search-tab').on('click', 'a', function(e){
        	...
        	$(this).closest('ul').find('input').val(active_value);
        });
    });
</script>