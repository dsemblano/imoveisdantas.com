...
<head>
    ...
    <?php $favicon = wp_get_attachment_image_url( $es_native_options->favicon_attachment_id,
            'original' );?>
	<link rel="shortcut icon"
	      href="<?php echo ! empty( $favicon ) ? $favicon : ES_NATIVE_URL . 'assets/images/favicon.ico'; ?>">
	<?php wp_head(); ?>
    <style>
...
<header class="native__color--background <?php if (  $es_native_options->is_sticky ) : ?>sticky-header<?php endif;?>">
    <div class="navigate-line <?php if ( $es_native_options->search_style == 'wide' && is_page_template('page-templates/page-with-top-search.php') ):?>color-menu<?php endif;?>">
		<div class="header__info-wrap">
			<a class="header__logo native__dark--background" href="<?php echo site_url(); ?>">
				<?php if ( !empty( $es_native_options->logo_attachment_id ) ):?>
					<img class="header__logo-image" src="<?php echo wp_get_attachment_image_url( $es_native_options->logo_attachment_id,
						'original' ); ?>"/>
				<?php else:?>
