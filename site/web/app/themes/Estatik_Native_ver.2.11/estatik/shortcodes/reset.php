...
<?php if ( ! is_user_logged_in() ) : ?>
    <h2 class="es-login__wrap-name"><?php _e( 'Restore password', 'es-native' ); ?></h2>
		...
		<form action="" method="post">
				...
				<label for="user_login">
					...
					<?php if ( $valid ) : ?>
						<input type="password" required name="pwd"
						       placeholder="<?php _e( 'Enter new password', 'es-native' ); ?>">
					<?php else: ?>
						<input type="text" name="user_login"
						       placeholder="<?php _e( 'Username or email address', 'es-native' ); ?>">
					<?php endif; ?>
				</label>
			</div>
			...
			<div class="es-submit__wrap">
				<input type="submit" class="es-btn es-btn-orange"
				       value="<?php _e( 'Get new password', 'es-native' ); ?>">
			</div>
