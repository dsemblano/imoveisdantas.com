<?php
// Add custom CSS to
function gf_custom_css_in_widget_form( $widget, $return, $instance ) {
	if ( !isset( $instance['css_class'] ) ) $instance['css_class'] = null;
	echo "<p><label for='widget-{$widget->id_base}-{$widget->number}-css_class'>". esc_html__( 'CSS Class', 'g5-beyot' ) .":</label>
...
function gf_custom_css_widget_update_callback( $instance, $new_instance ) {
...
function gf_custom_css_add_widget_css_class( $params ) {
...
// Front end hook
function gf_custom_css_frontend_hook() {
	if ( !is_admin() ) {
		add_filter( 'dynamic_sidebar_params','gf_custom_css_add_widget_css_class' );
	}
}

add_action( 'in_widget_form', 'gf_custom_css_in_widget_form', 10, 3 );
add_filter( 'widget_update_callback', 'gf_custom_css_widget_update_callback', 10, 2 );
add_action( 'wp_loaded', 'gf_custom_css_frontend_hook' );